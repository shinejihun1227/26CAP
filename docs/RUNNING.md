...
**오른발이 안 잡히거나 IP를 직접 등록해야 하면 [양발 연결 및 오른발 문제 해결](RIGHT_INSOLE_CONNECTION.md)을 참고하세요.** 핫스팟의 접속 장치 수와 웹의 센서 등록 상태는 별개입니다. IP 입력창과 발 선택 드롭다운은 자동 갱신 중에도 유지되며, 이번 웹 화면 수정은 브라우저에서 `Ctrl+F5`로 반영할 수 있습니다.
