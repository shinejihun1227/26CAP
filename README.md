...
## 보행 측정 상태 관찰 — 2026-09-24

**보행 측정 → 상태 관찰**에 `FoG 모니터링 / 발 건강 모니터링`을 추가했습니다. FoG 신호 구간·관찰 지속시간과 양발 온도·습도·상대 압력 변화를 나누어 확인합니다. 이번 웹 접속의 최근 5·15·30분을 표시하며 새로고침하면 초기화됩니다. 일별 기록 저장은 기존 데이터 관리에서 진행합니다. [화면 사용법과 집계·저장 범위](docs/OBSERVATION_MONITORING.md)

## 관절 카메라 화면 사용법 — 2026-09-23

1. **관절 움직임**에서 ‘웹캠 사용에 동의합니다’를 체크한 뒤 **웹캠 켜기**를 누릅니다.
2. 관절·동작과 자세를 고릅니다. 자동으로 표시된 촬영 방향을 향하고, 선택한 관절과 어깨·골반이 보이게 합니다. ‘화면에는 나만 있어요’를 확인합니다.
3. 준비 완료가 되면 **15초 기록 시작**을 누르고 편안한 범위에서 움직입니다. 완료 후 **기록 저장**을 누릅니다.
4. 아래의 **측정값 한눈에 보기**에서 각도·관찰 범위·기록 품질을 봅니다. 기록 품질은 유효 표본 비율이며 움직임 안정도나 건강 점수가 아닙니다. 그래프에서 관절 인식을 놓친 구간은 끊겨 표시됩니다.
5. **저장 기록 비교**에서 기록을 불러옵니다. 같은 사람·관절·방향·자세·촬영 환경·신뢰도·모델·화면 비율의 이전 품질 통과 기록만 비교할 수 있습니다. 범위·중앙 각도·유효 표본 비율을 나란히 보여 줍니다. 불러오기는 카메라를 끄고 해당 기록의 촬영 설정으로 맞춥니다.

측정 코드와 인식 신뢰도는 **추가 설정**에 있습니다. 같은 사람은 같은 측정 코드를 사용하고, 카메라 위치가 바뀌면 촬영 환경 코드를 바꿉니다. 각도 정의와 외부 각도계 비교 기능도 접힌 설명에서 사용할 수 있습니다. 웹에서 **Ctrl+F5**로 새 화면을 불러오면 되며, ESP32 재업로드나 AI 모델 재설치는 필요하지 않습니다.

검증: 전체 자동 검사 **156개 통과**. 실제 기록과 분리된 임시 합성 기록으로 불러오기·비교 해제·비교 그래프·관찰 요약을 확인했고, PC와 390px 모바일 배치를 점검했습니다. 이번 화면 검사에서 실제 착용자 영상을 촬영하지 않았습니다.

## 2026-09-23 가독성 개선

- **오늘 요약:** 연결 상태를 먼저 보고, 걸음 수·좌우 균형·신발 안 상태를 큰 카드로 확인합니다. AI 상태와 상세 설명은 그 아래에 있습니다.
- **관절 움직임:** 카메라 준비 → 몸 위치 확인 → 15초 기록의 진행 상태를 보여 줍니다. 준비가 끝나기 전에는 기록 버튼 옆에 필요한 조건이 표시됩니다.
- **변화 보기:** 월별 달력에서 실제 저장한 날짜를 선택하면 비교 기준 날짜가 바뀝니다. 보라색은 관절 기록, 초록색은 보행 기록이며 건강 상태 등급을 뜻하지 않습니다. 최근 저장 날짜도 함께 표시합니다.
- **데이터 관리:** 세트·기록 내보내기와 다섯 동작 CSV 수집을 기존과 같이 사용합니다. 전체 메뉴의 글자·여백·버튼 색상을 통일하고 휴대폰 화면에도 맞췄습니다.
- 메뉴를 바꾸면 새 화면의 맨 위로 이동합니다. 센서 갱신 중에는 펼친 설명과 입력 상태를 유지합니다.
- 이미 실행 중인 웹은 **Ctrl+F5**로 새 화면을 불러오세요. 이번 변경으로 ESP32를 다시 업로드하거나 AI 모델을 재설치할 필요는 없습니다.
- 검증: 웹·HTTP·관절 기록·CSV·실제 RF/CNN 합성 데이터 연동 검사 **151개 통과**, 실패·건너뛴 검사 0개. PC 및 390px 모바일 화면의 배치·메뉴 이동·달력 선택을 확인했습니다. 실물 센서 측정은 이번 UI 검사의 대상이 아닙니다.

## 다섯 동작 수집과 FoG 지속 출력 업데이트

- **데이터 관리 → 다섯 동작 수집 안내 · CSV 분석**에서 25초 보정과 다섯 동작을 안내받고, 동작별 저장 횟수를 확인할 수 있습니다.
- FoG 신호 감지 동안 해당 발의 **진동·레이저를 유지**하고, 해제되면 끕니다. 출력 중지·재시작은 기기 설정과 보행 측정 화면에 있습니다.
- **양쪽 ESP32에 최신 펌웨어를 다시 업로드**하고 PC 프로그램을 재시작하세요. 웹만 새로고침하면 펌웨어는 바뀌지 않습니다.
- 자세한 수집 순서·배선·실행·출력 동작은 [수집 및 FoG 출력 사용법](docs/GUIDED_COLLECTION_AND_FOG_CUE.md)을 확인하세요.
...
## 2026-09-22 사용자 화면 개편

자주 쓰는 측정 화면과 자료 관리 화면을 분리했습니다. 기존 데이터와 세트는 그대로 사용합니다.

| 메뉴 | 이곳에서 하는 일 |
|---|---|
| 오늘 요약 | 깔창 연결 상태, AI 상태, 걸음 수·좌우 균형·온습도 확인 |
| 보행 측정 | 현재 발 압력과 보행 상태 확인 |
| 관절 움직임 | 웹캠으로 15초 측정하고 결과 저장 |
| 변화 보기 | 같은 사람·같은 조건의 날짜별 기록 비교 |
| 기기 설정 | 왼발·오른발 연결, 개인 기준 설정·AI 보정, 센서·출력 점검 |
| 데이터 관리 | 관절 기록을 세트로 묶기, 보행 기록 저장, CSV/JSON 내보내기·삭제 |

- **관절 측정:** ‘관절 움직임’에서 웹캠 동의 → 웹캠 켜기 → 관절·촬영 방향 선택 및 확인 → 15초 기록 → 기록 저장. 사용자 코드와 촬영 환경은 ‘내 기록’에서 바꿉니다. 기록 버튼 위에서 현재 준비 상태를 확인할 수 있습니다.
- **관절 세트:** ‘데이터 관리’에서 기록을 저장한 사용자 코드를 확인하고 세트를 만들거나 선택합니다. 아래 ‘방향별 기록’에서 이미 저장한 기록을 세트에 추가한 뒤 통합 JSON/CSV로 내보냅니다. 세트를 먼저 만들 필요 없이 개별 측정부터 시작할 수 있습니다.
- **보행 기록 저장:** ‘보행 측정’의 ‘측정 기록 저장’을 누르면 데이터 관리의 보행 저장 영역으로 이동합니다. 실제 센서 연결 상태에서 사용자 코드·센서 환경 코드·저장 동의를 확인하고 ‘센서 기록 시작’을 누릅니다. 다른 메뉴로 이동하거나 탭을 숨기면 기록이 중지됩니다. 저장된 자료는 유지됩니다.
- **원시 IMU CSV와 모델 분석:** 데이터 관리 상단의 ‘다섯 동작 수집 안내 · CSV 분석’에서 기존 `/data/` 기능을 그대로 사용할 수 있습니다.
- **기술 정보:** 모델별 점수, 센서 세부 구성, 전문 각도계 비교 등은 해당 화면의 ‘자세히 보기’에서 펼칩니다. 휴대폰 하단에는 자주 쓰는 네 메뉴, 상단에는 기기 설정·데이터 관리가 있습니다.

이번 화면 변경은 웹 새로고침으로 적용됩니다. 진행 중이거나 미저장인 관절 기록을 먼저 저장한 뒤 **Ctrl+F5**를 누르세요. 저장 자료는 서버 PC에 남으며, 화면 이동 시 현재 사용자 코드와 측정 설정도 유지합니다. 브라우저를 완전히 새로 열 때는 사용자 코드를 다시 확인하세요.

**검증:** 웹·저장·실제 RF/CNN 연동을 포함한 전체 검사 137개가 통과했습니다. PC 및 390px 휴대폰 화면에서 메뉴 이동, 데이터 관리 분리, 측정자 코드 유지와 가로 넘침 여부를 확인했습니다.
...
펌웨어를 열고 `firmware/stepon_c3/04_2_sta_bilateral_wroom/config.h` 맨 위의 한 줄만 바꿉니다. 기본값은 기존 배선과 호환되는 4개 버전입니다.

```cpp
#define STEPON_SENSOR_PROFILE STEPON_SENSOR_PROFILE_4  // 센서 4개
// #define STEPON_SENSOR_PROFILE STEPON_SENSOR_PROFILE_2  // 센서 2개
```

압력과 온습도 센서는 같은 버전 수를 사용합니다. 왼발·오른발 보드 모두 같은 값을 선택하고, 각 보드에 펌웨어를 다시 업로드하세요. 웹과 PC AI 프로그램의 설치 설정은 바꿀 필요가 없습니다. 웹은 펌웨어가 보낸 버전을 자동으로 알아냅니다. 이 웹 코드를 받은 뒤에는 실행 중인 프로그램을 `run.bat --restart`로 한 번 다시 시작하고 브라우저에서 Ctrl+F5를 누르세요.

| 구성 | 압력 배선 | 온습도 배선 | 웹 표시와 해석 |
|---|---|---|---|
| 4개 | 기존 CD74HC4067: SIG→GPIO34, S0/S1/S2/S3→32/33/4/26, 센서는 C0/C2/C4/C6 | 기존 TCA9548A CH3/4/5/6, 공통 SDA/SCL 13/14 | P1~P4와 T1~T4가 각각 독립 측정됩니다. 위치별 압력 분석을 사용할 수 있습니다. |
| 2개 | MUX를 우회하고 FSR 출력 1→GPIO34, FSR 출력 2→GPIO35에 각각 전압분배 회로로 연결 | TCA를 우회하고 SHTC3 1→SDA13/SCL14, SHTC3 2→별도 SDA21/SCL22 | 압력 센서1 값은 P1·P2, 센서2 값은 P3·P4에 복사 표시됩니다. 온습도 센서1은 T1·T2, 센서2는 T3·T4에 복사 표시됩니다. 각 쌍은 같은 원본값이라 표시 차이는 0.0입니다. |

2개 버전의 FSR도 센서마다 기존처럼 **3.3V → FSR → GPIO 입력점**, 입력점에서 **10kΩ → GND**로 전압분배를 만듭니다. ESP32·센서·저항의 GND는 공통이어야 하며 GPIO 입력에 5V를 넣지 마세요. 이 모드에서는 CD74HC4067과 TCA9548A를 데이터 경로에서 사용하지 않습니다. 공유된 P/T 위치는 별도 측정 위치가 아니므로 웹에서 위치별 압력중심·착지·추진 판정을 하지 않습니다. 양발 하중 총량, IMU 움직임, 실제 온습도 센서 두 개의 변화는 계속 관찰합니다.
...
| 베이스 저항 → 2N2222 Base | GPIO27, AI 감지 동안 레이저 유지 |
...
WROOM-DA의 GPIO2·25는 안테나 핀입니다. 현재 설정은 이 두 핀을 사용하지 않습니다. PC의 실시간 AI가 해당 발에서 FoG 신호 감지(confirmed)를 유지하는 동안 진동과 레이저를 지속하고, 감지가 해제되면 끕니다. `ENABLE_LASER_OUTPUT=true`이며 최신 펌웨어 업로드와 개인 IMU 보정이 필요합니다. 명령 갱신이 끊기면 보드는 마지막 명령 후 1.5초 안에 자동으로 끕니다. CSV 분석은 실제 출력을 제어하지 않습니다.
...
**‘15초 기록 시작’이 회색일 때:** 버튼 바로 위에 표시되는 대기 이유를 확인하세요. `http://127.0.0.1:8000/?view=mediapipe&esp32=1&transport=sta&ai=1&mobile=0`을 노트북에서 열고 다음 순서로 진행합니다.

1. 카메라 아래 **‘웹캠 사용에 동의합니다’**를 체크하고 **‘웹캠 켜기’**를 누릅니다. 브라우저가 카메라 권한을 요청하면 허용합니다.
2. 사용자 코드·촬영 환경 코드를 입력하고 관절·자세·촬영 방향을 선택합니다.
3. 몸을 선택한 방향으로 돌린 후 **‘몸의 선택한 방향을 카메라로 향했고, 화면에는 한 사람만 있습니다’**를 체크합니다.
4. 선택한 관절과 어깨·골반이 화면에 보이게 조정합니다. 버튼 위에 **‘준비 완료’**가 나오면 **‘15초 기록 시작’**을 누르고, 완료 후 **‘기록 저장’**을 누릅니다.

사용자 코드·환경 코드·신뢰도 변경은 방향 확인을 해제하지 않습니다. 관절·자세·방향을 바꾸면 다시 확인해야 합니다. 인식이 안 되거나 지연된 표본은 유효 측정으로 사용하지 않습니다. 이 기능의 기록 시작에는 ESP32 연결이 필요하지 않습니다. 코드 업데이트 후 Ctrl+F5로 새로고침하면 적용되며, 진행 중이거나 저장하지 않은 각도 기록은 먼저 저장하세요.
