...
> 현재 4-2 설정은 **SDA=GPIO13, SCL=GPIO14, MUX S2=GPIO4(D4), 압력=C0/C1/C2/C3**입니다. 실제 배선과 맞춰 양쪽 보드에 다시 업로드하세요. `PC_HOST`는 노트북의 현재 Wi-Fi IPv4로 설정합니다.
...
| 압력 4채널 | C0 앞쪽 / C1 가운데 안쪽 / C2 가운데 바깥쪽 / C3 뒤꿈치 |
...
**현재 배선으로 업데이트:** USB와 배터리를 분리하고 S2=GPIO4, 압력 4개=C0/C1/C2/C3을 확인합니다. `config.h`는 `MUX_S2 = 4`, `PRESSURE_CHANNELS[4] = {0, 1, 2, 3}`이어야 합니다. 왼발은 `STEPON_RIGHT_FOOT=0`, 오른발은 `1`로 각각 업로드합니다. 웹 서버를 재시작하고 브라우저를 Ctrl+F5로 새로고침합니다. AI 모델 재설치는 필요하지 않습니다.
...
현재 배선은 S2=GPIO4, 압력 C0/C1/C2/C3입니다. 구버전은 S2=GPIO18, C0/C2/C4/C6을 읽으므로 현재 배선에 맞는 4-2를 다시 업로드합니다. 웹도 함께 업데이트하고 서버를 다시 시작한 뒤 브라우저를 Ctrl+F5로 새로고침합니다. 기존 C0/C2/C4/C6 보드는 계속 지원하며 웹은 보드가 보낸 실제 채널을 표시·기록합니다.

시리얼 모니터 115200에서 2초마다 `[PRESSURE] S0=32 S1=33 S2=4 S3=26 SIG=34 raw C0=... C1=... C2=... C3=...`를 확인합니다. 아무것도 누르지 않은 상태와 센서 하나씩 누른 상태를 비교합니다. 이 값은 0~4095 ADC 원시값이며, 웹의 0~100은 상대값이지 kg이나 뉴턴이 아닙니다.
...
- S0=GPIO32, S1=GPIO33, S2=GPIO4, S3=GPIO26, SIG=GPIO34. C0~C3 선택에는 S0와 S1이 바뀌며 S2/S3는 LOW입니다.
- 각 센서는 `3.3V → FSR → 해당 C채널`, 해당 C채널에서 `10kΩ → GND`로 구성합니다. 네 채널의 신호 노드와 저항은 서로 독립적이어야 합니다. 3.3V/GND 전원선만 공통으로 연결합니다.
- C0~C3이 브레드보드 같은 도통 줄에 묶이지 않았는지, 각 채널의 GND 저항이 빠지지 않았는지 확인합니다. 떠 있는 ADC 입력은 이전 채널의 영향을 받을 수 있습니다.
...
**GPIO4·C0~C3 배선 반영 검사:** 웹 전체 129개 검사가 통과했습니다. 새 채널의 단일 센서 값이 다른 표시로 복제되지 않는지, 구버전 채널 보드와 함께 사용할 때 각 발의 채널을 유지하는지, 기록의 채널 정보가 보존되는지를 포함합니다. ESP32 Dev Module 컴파일도 성공했습니다(Flash 853,713 bytes, 정적 RAM 46,056 bytes). 이는 코드·통신 형식 검사 결과이며 압력 센서의 물리적 배선, 누름에 따른 독립 반응, 무선 연결 안정성이 확인됐다는 뜻은 아닙니다.
