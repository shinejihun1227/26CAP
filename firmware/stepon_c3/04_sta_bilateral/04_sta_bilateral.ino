...
String stateJson() {
  ...
  s = "{\"device\":\"StepOn-C3\",\"firmware\":\"04_sta_bilateral\",\"wifi_mode\":\"STA\",\"device_id\":\"" + deviceId + "\",\"boot_id\":\"" + bootId + "\",\"foot_side\":\"" + FOOT_SIDE + "\",\"bilateral_available\":false";
...
void registrationTask(void *) {
  ...
  while (true) {
    if (WiFi.status() == WL_CONNECTED) {
      ...
      const int code = http.POST("{\"firmware\":\"04_sta_bilateral\",\"foot_side\":\"" + String(FOOT_SIDE) + "\",\"device_id\":\"" + deviceId + "\"}");
...
void setup() {
  Serial.begin(115200); Serial.setTxTimeoutMs(10); delay(800);
  pinMode(LASER_PIN, OUTPUT); digitalWrite(LASER_PIN, LOW);
  bootId = String(esp_random(), HEX);
  deviceId = "c3-" + String(uint32_t(ESP.getEfuseMac() >> 32), HEX) + String(uint32_t(ESP.getEfuseMac()), HEX);
  Serial.printf("\nStepOn 04_sta_bilateral foot=%s reset=%d\n", FOOT_SIDE, int(esp_reset_reason()));
  ...
  server.on("/api/ping", HTTP_GET, [] { sendJson(200, "{\"ok\":true,\"firmware\":\"04_sta_bilateral\",\"foot_side\":\"" + String(FOOT_SIDE) + "\",\"device_id\":\"" + deviceId + "\"}"); });
