...
**현재 웹은 WROOM을 지원합니다.** 실시간 Wi-Fi 연결에는 이 **4-2** 코드를 업로드하세요. `06_bmi270_csv_wroom`은 USB CSV 수집용이므로 웹에 Wi-Fi로 자동 등록되지 않습니다. 06으로 기록한 CSV는 웹 `/data/`에서 별도로 분석할 수 있습니다. [오른발 연결·IP 등록 안내](../../../docs/RIGHT_INSOLE_CONNECTION.md)에서 보드별 설정을 확인하세요.
...
- RF/CNN은 PC에서 실행합니다. ESP32에는 이 펌웨어만 업로드하며 Python 모델을 넣지 않습니다. 웹의 AI용 CSV는 가속도·자이로 원시 6축과 장치 시각을 기록합니다. **현재 RF/CNN의 직접 입력은 보정·정규화한 가속도 3축**이며, RF는 여기서 특징 6개를 계산합니다. 웹의 25초 보정은 yaw 축을 식별하지 않으므로 자이로 회전 억제는 비활성입니다. 압력·온습도는 웹에서 별도로 표시하고 현재 IMU 분석 CSV나 RF/CNN 점수에 합치지 않습니다. 자세한 CSV 형식과 점수 정의는 아래 문서를 따릅니다.
