...
test('all Arduino sketch-local headers share four-channel API and thermal mapping', () => {
  const headers = ['01_sensor_test/sensor_core.h', '02_wifi_api/sensor_core_local.h', '03_final/sensor_core_local.h'].filter((file) => fs.existsSync(new URL(`../../firmware/stepon_c3/${file}`, import.meta.url))).map((file) => fs.readFileSync(new URL(`../../firmware/stepon_c3/${file}`, import.meta.url), 'utf8'));
  assert.ok(headers.length >= 1);
  for (const code of headers) {
    assert.match(code, /PRESSURE_COUNT = 4;/); assert.match(code, /PRESSURE_CHANNELS\[PRESSURE_COUNT\] = \{0, 2, 4, 6\}/);
    assert.match(code, /THERMAL_CHANNELS\[THERMAL_COUNT\] = \{3, 4, 5, 6\}/);
    for (const field of ['pressure_count', 'pressure_layout', 'pressure_channels', 'shtc3_channels']) assert.ok(code.includes(field));
  }
  if (headers.length > 1) assert.equal(headers[0], headers[1]);
  for (const step of ['02_wifi_api', '03_final'].filter((s) => fs.existsSync(new URL(`../../firmware/stepon_c3/${s}`, import.meta.url)))) assert.match(fs.readFileSync(new URL(`../../firmware/stepon_c3/${step}/sensor_core.h`, import.meta.url), 'utf8'), /#include "sensor_core_local.h"/);
});
