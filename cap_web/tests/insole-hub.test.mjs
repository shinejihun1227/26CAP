...
test('packaged WROOM sketch is self-contained STA only and credentials are ignored', () => {
  const root = new URL('../../firmware/stepon_c3/04_2_sta_bilateral_wroom/', import.meta.url);
  const code = fs.readFileSync(new URL('04_2_sta_bilateral_wroom.ino', root), 'utf8');
  ...
  assert.match(fs.readFileSync(new URL('../../.gitignore', import.meta.url), 'utf8'), /04_2_sta_bilateral_wroom\/wifi_secrets.h/);
});
