...
export function renderTrendsContent() {
  return `<div class="trends-workspace" data-trends-root>
    <header class="trends-heading"><div><span class="trends-kicker">MY DAILY CHANGES</span><h1>변화 추이</h1><p>어제와 지난 기록을 비교하고, 다시 확인할 변화를 찾으세요.</p></div><span class="trends-private">내 PC 저장 · 한국 날짜 기준</span></header>
    <section class="trends-toolbar" aria-label="비교할 기록 선택">
      ...
      <button type="button" data-trend-action="refresh">새로고침</button><button type="button" data-trend-action="export" disabled>내 기록 JSON</button>
    </section>
    <p class="trends-notice" role="status" data-trend-notice>저장 기록을 확인하고 있습니다. 8001은 편집용 미리보기이며 실제 비교는 8000에서 확인하세요.</p>
    <section class="trends-summary" aria-label="변화 요약"><div><span class="trends-kicker">SUMMARY</span><h2>기록이 쌓이면 변화가 보입니다</h2><p>실제 저장된 기록만 비교합니다. 데이터가 없는 날은 0으로 채우지 않습니다.</p></div><div class="trends-summary-count"><strong data-trend-days>—</strong><span>기록이 있는 날</span></div></section>
    <section class="trends-panel trends-rom"><div class="trends-panel-head"><div><span class="trends-kicker">MEDIAPIPE · SAME CONDITIONS</span><h2>나의 관절 움직임</h2></div><button type="button" data-view="mediapipe">관절 측정하기 →</button></div>
    ...
    <section class="trends-panel trends-sensors"><div class="trends-panel-head"><div><span class="trends-kicker">SENSORS & ALGORITHMS</span><h2>보행·발 상태 기록</h2></div><span data-trend-live-status>기록 중 아님</span></div>
      <div class="trends-recorder"><label>센서 환경 코드<input data-trend-setup value="보행-기본" maxlength="60" /></label><label class="trends-consent"><input type="checkbox" data-trend-consent />이 측정 코드로 실제 센서·알고리즘 관찰값을 PC에 30일 저장합니다.</label><button type="button" data-trend-action="start" disabled>센서 기록 시작</button><button type="button" data-trend-action="stop" disabled>기록 중지</button></div>
      <p class="trends-help">이 탭이 보이는 동안 약 1초마다 저장합니다. 탭을 숨기거나 다른 메뉴로 이동하면 중지됩니다. 시연값·연결 끊김·오래된 값은 제외하며, 기록 중 화면은 5초마다 갱신됩니다. 신발·센서 위치·운동 종류·모델 가중치 또는 보정 파일을 바꾸면 새 환경 코드를 사용하세요.</p>
    ...
    <aside class="trends-caution"><strong>변화와 건강 판정은 다릅니다.</strong><p>각도 범위 증가·감소, AI 판정 비율 변화는 관찰 결과입니다. 치료 효과나 악화 여부를 자동 확정하지 않습니다. 촬영 자세·신발·장비·보정값·모델을 바꾸면 비교 조건도 달라집니다. 통증이나 불편이 있으면 측정을 중단하고 전문가에게 기록을 보여 주세요.</p><p>관절·센서 기록은 서버 PC의 비공개 폴더에 저장됩니다. 같은 PC의 다른 사용자도 볼 수 있으며 로그인으로 분리된 의료 기록 시스템은 아닙니다. 카메라 영상은 저장하지 않습니다.</p></aside>
  </div>`;
}
