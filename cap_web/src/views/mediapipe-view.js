...
export function renderMediaPipeContent({ manage = false } = {}) {
  ...
  return `<section class="rom-workspace ${manage ? "is-management" : "is-measurement"}" data-rom-root data-rom-mode="${manage ? "manage" : "measure"}">
    <header class="rom-heading"><div><span class="rom-eyebrow">${manage ? "관절 자료" : "카메라로 간편하게"}</span><${manage ? "h2" : "h1"}>${manage ? "관절 기록과 측정 세트" : "나의 관절 움직임"}</${manage ? "h2" : "h1"}><p>${manage ? "저장한 기록을 골라 세트로 묶거나 내보내세요." : "관절을 고르고, 15초 동안 움직인 뒤 결과를 확인하세요."}</p></div><div class="rom-heading-actions"><span class="rom-local-badge">PC 안에서 분석 · 영상 저장 안 함</span><div class="simple-route-link"><button type="button" data-view="${manage ? "mediapipe" : "records"}">${manage ? "새 관절 측정하기 →" : "저장한 기록 · 세트 관리 →"}</button></div></div></header>
    ${manage ? '' : `<ol class="rom-preparation" aria-label="관절 측정 준비 순서"><li data-rom-step="camera"><span class="step-number">1</span><div><b>카메라 준비</b><p>사용에 동의한 뒤 웹캠을 켜 주세요.</p></div><small data-rom-step-state>대기</small></li><li data-rom-step="pose"><span class="step-number">2</span><div><b>몸 위치 확인</b><p>선택한 관절과 어깨·골반을 화면에 넣으세요.</p></div><small data-rom-step-state>대기</small></li><li data-rom-step="record"><span class="step-number">3</span><div><b>15초 움직임 기록</b><p>준비 완료가 보이면 기록을 시작하세요.</p></div><small data-rom-step-state>대기</small></li></ol>`}
    ...
    <div class="rom-layout ${manage ? "rom-management-layout" : ""}">
      <section class="rom-camera-panel" aria-label="웹캠 관절 분석" ${manage ? "hidden" : ""}>
        ...
        <details class="simple-details"><summary>실시간 각도 그래프</summary><div class="rom-trace"><canvas data-rom-chart width="800" height="130" aria-label="선택한 관절의 최근 각도 변화"></canvas><small>최근 각도 변화 · 2D 추정값 · °</small></div></details>
      </section>
      <section class="rom-setup-panel" aria-label="측정 설정과 기록">
        <div class="rom-panel-head"><h2>${manage ? "기록 선택과 결과" : "15초 움직임 측정"}</h2></div>
        <details class="simple-details rom-identity" ${manage ? "open" : ""}><summary data-rom-identity>내 기록 · P01</summary>
        ...
        <p id="rom-participant-note" class="rom-participant-help"><b>P01 = 한 사람의 기록 이름표</b><br>예: 본인 P01, 팀원 P02. 같은 사람은 날짜·관절·촬영 방향이 달라도 같은 코드를 쓰세요. 프로그램 코드나 진단 번호가 아니며 로그인 비밀번호도 아닙니다. 실명 대신 코드를 사용하고, 카메라 위치·거리·높이가 바뀌면 촬영 환경 코드를 바꾸세요.</p></details>
        <div ${manage ? "hidden" : ""}>
        ...
        <details class="simple-details"><summary>선택한 관절의 촬영 방법</summary><section class="rom-joint-guide" data-rom-joint-guide aria-label="선택 관절의 촬영 방법과 각도 정의" aria-live="polite">${renderJointGuide('left_shoulder')}</section></details>

        <label class="rom-check"><input type="checkbox" data-rom-direction-confirmed> 몸의 선택한 방향을 카메라로 향했고, 화면에는 한 사람만 있습니다.</label>
        <details class="simple-details"><summary>인식 설정</summary><label class="rom-confidence">관절 신뢰도 하한 <select data-rom-config="confidence"><option value="0.65">65% · 낮음</option><option value="0.75" selected>75% · 기본</option><option value="0.85">85% · 엄격</option></select></label></details>
        <div class="rom-record-box"><div><b data-rom-record-title>15초 움직임 기록</b><span data-rom-progress-text>0 / 15초</span></div><progress data-rom-progress max="15" value="0" aria-label="기록 진행률"></progress><p>편안한 범위 안에서 천천히 움직이세요. 통증·어지러움이 있으면 즉시 중단하세요.</p><p class="rom-quality" id="rom-record-help" data-rom-record-help role="status">웹캠 동의 → 웹캠 켜기 → 촬영 방향 확인 → 관절 인식 순서로 준비하세요.</p><div class="rom-buttons"><button type="button" class="rom-primary" data-rom-action="record" aria-describedby="rom-record-help" disabled>15초 기록 시작</button><button type="button" class="rom-danger" data-rom-action="abort" disabled>통증·불편 / 중단</button></div></div>
        </div>
        <div class="rom-result" data-rom-result>아직 완료된 기록이 없습니다. 기록 완료 후 범위와 품질을 확인할 수 있습니다.</div>
        <div class="rom-buttons"><button type="button" class="rom-primary" data-rom-action="save" ${manage ? "hidden" : ""} disabled>기록 저장</button><button type="button" data-rom-action="baseline" disabled>개인 기준으로 지정</button><button type="button" data-rom-action="discard" ${manage ? "hidden" : ""} disabled>미저장 기록 버리기</button><button type="button" data-rom-action="save-alone" hidden disabled>개별 기록으로 저장</button></div>
