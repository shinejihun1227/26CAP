...
export function renderMediaPipeContent({ manage = false } = {}) {
  ...
  return `<section class="rom-workspace ${manage ? "is-management" : "is-measurement"}" data-rom-root data-rom-mode="${manage ? "manage" : "measure"}">
    <header class="rom-heading"><div><span class="rom-eyebrow">${manage ? "관절 자료" : "카메라로 간편하게"}</span><${manage ? "h2" : "h1"}>${manage ? "관절 기록과 측정 세트" : "나의 관절 움직임"}</${manage ? "h2" : "h1"}><p>${manage ? "저장한 기록을 골라 세트로 묶거나 내보내세요." : "관절을 고르고, 15초 동안 움직인 뒤 결과를 확인하세요."}</p></div><span class="rom-local-badge">PC 안에서 분석 · 영상 저장 안 함</span></header>
    ...
    <div class="simple-route-link"><button type="button" data-view="${manage ? "mediapipe" : "records"}">${manage ? "새 관절 측정하기 →" : "저장한 기록 · 세트 관리 →"}</button></div>
